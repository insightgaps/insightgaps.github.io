...
> **RESOLVED — 2026-09-03, by owner instruction ("delete the slum report").**
> Executed as a clean unpublish through the build system: manifest, page body, stylesheet, dataset (`slum_fires_data.json`), and validator route removed; sitemap/indexes/archive/homepage stats regenerated (3 investigations / 120 sources); permanent redirects added (`/investigations/dhaka-slum-fires/*` and its legacy `/content/` path → `/investigations/`); the hard-coded homepage slug filter removed from build code (also resolving REP-015's code-level residue; D-5 is now moot for this work). Commit: see git log 2026-09-03. History preserved in git (the report remains retrievable). **Optional follow-up (owner):** whether the removal should also be recorded as a line in the public corrections log — left unlogged per the doctrine that corrections entries are editor-authored.

- **QUESTION (original):** Given the audit findings (fabricated data lineage — the registered raw CSV's logged SHA-256 is the hash of an empty file; the "cleaning script" hardcodes the dataset; the central legal claim carries no case citation in public; hero 16.2% metric has no data; n=4 "clear correlation" language; empty governance templates; AI-signed sign-off): keep published as-is, publish with annotations (limitations section + sourcing disclosures + restored claim drawer), or unpublish pending the raw evidence?
