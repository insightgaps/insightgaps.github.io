...
READABILITY_FLOOR = (
    "\n  <style>/* Phase 4/6 presentation repairs (audit REP-013 + §15). Presentation-only; no content altered. "
    "Mobile: readability floor for micro inline type; sub-nav wrapping; canvas scaling; "
    "table horizontal scroll; scroll containment for animation tracks; and clamping of "
    "inline-styled fixed-width/grid-min elements (e.g. minmax(280px,...) grids, 280px "
    "panels inside narrower containers) that caused the <=414px horizontal overflow. "
    "Chart geometry is untouched: clamps only allow shrink-to-fit, never growth. */"
    ...
    " .counter-right, .cr-item, .tier { max-width: 100% !important; min-width: 0 !important; overflow-wrap: anywhere !important; }"
    " .surge-grid { grid-template-columns: 1fr !important; }"
    " main div, main span, main ul, main li { max-width: 100% !important; box-sizing: border-box !important; }"
    " #sources-list, .src-item, .sb, .surl, .sid { max-width: 100% !important; overflow-wrap: anywhere !important; word-break: break-word !important; min-width: 0 !important; }"
    " [style*='width:280px'], [style*='width: 280px'] { width: 100% !important; max-width: 100% !important; }"
    " [style*='minmax(280px'] { grid-template-columns: minmax(0, 1fr) !important; }"
    "}</style>"
)
