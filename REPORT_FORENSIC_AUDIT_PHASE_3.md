...
**Strong:** arithmetic integrity of the flagship ratios is genuinely good — **every calculable published figure reproduces exactly from stated inputs** (0.46%, 1.48%, 92.3%, 11,896, 3.1×, 52.3%, 265.5%, 55.06%, 26.6 years, 12/day, 193+700+1,456=2,349); the master evidence workbook is internally coherent and **checksum-matches its manifest exactly** (92,924 bytes, SHA-256 verified); `clean_data.py` is a faithful lossless raw→cleaned pipeline (132/12/60/18/2/18/25/65/12 rows all reconcile); honest practices worth crediting: the cross-country denominator warning, the OCC-scope note, the CEDAW disposal-vs-conviction warning, the M-006 data-conflict log, §17 framed carefully as a 2-case sample with CONFIRMED ABSENT national tracking, composite-story disclosure, and the reasoned removal of a live counter in favor of a static sourced figure.
**Weak (full audit incorporated):** the entire evidence chain terminates at press reports and AI compilation — **zero primary documents archived**; the evidence ledger's five checksummed `secure_path`s point to an `assets/BD-INV-002/` folder that **does not exist**; the master evidence file is linked 6+ times publicly ("Every figure in this report is in the master evidence file") and **404s**; the source count is **91 on every page vs 65 in the actual registry** (verified three ways), with pages citing S-26/27/28 on screen — IDs absent from the registry; **18 AGGREGATE rows are sourced to "Gemini-doc" with 14 carrying CONFIRMED** (violating the bureau's own CONFIRMED rule and Tier-E placement of AI compilations), including a 165,000-pending figure that contradicts the published 148,314/151,317 pair and was silently omitted; **page-to-page contradictions**: comparators (SA 72.2 vs 77.5; UK 53.4 vs 63.5; India 27.4 vs 22.7 — both sets labeled CONFIRMED), division IPV rates disagree on 5 of 6 divisions between report and detailed (methodology claims v2.0 "corrected" values while the flagship still shows the old set), tribunal count 101 vs XLSX holding 54/95/~100, BMP annual figures re-periodized as "H1 vs H1" in the tracker without source; "40 acquittals" published vs **18 actual** in the workbook; measure count stated three ways (8/10/12); period drift (2001–2023 vs 2001–2024); `verify_claims.py` prints "All claims successfully parsed and verified!" while Claim 2 never computes anything and Claim 4 "verifies" a figure published nowhere while the published 2,349-day figure goes untested (untestable — BRAC microdata absent); tracker says "Updated Weekly" while its own How-This-Works says "no fixed schedule" and it contains **zero fetch() calls** (static snapshot presented as a monitoring station; the described 6-hourly GitHub Actions collector does not exist); pre-publish verdict stamped 🟢 PASS over its own logged Stage 05: 0/3 and Stage 06: 0/12; self-audit docs affirmatively misdescribe artifacts (link-integrity ticked for a dead link; phantom leads.json schema/collector); the Race-disclosure note was added→reverted→re-added across three commits with no changelog; four version strings across artifacts with no mapping.
**Critical findings:** REP-031 (P0: master evidence file dead-linked 6+ times while self-audit ticked links-verified); REP-032 (P0: 91-vs-65 source count, plus citations to unregistered S-26/27/28); REP-033 (P1: zero archived primary documents; evidence ledger paths to nonexistent folder); REP-034 (P1: page-to-page statistical contradictions incl. comparator/IPV/tribunal/BMP sets); REP-035 (P1: pre-publish PASS over logged 0/3 + 0/12); REP-036 (P2: Gemini-doc CONFIRMED rows); REP-037 (P2: verify_claims.py demonstrational); REP-038 (P2: tracker static/cadence contradiction); REP-039 (P3: acquittal count 40-vs-18, measure-count confusion, period drift); REP-040 (P4: version fragmentation; self-audit formula error 0.000215 vs actual 0.000236).
**Tier-1 verdict:** the archived evidence package **does not meet the bureau's published Tier 1 bar** — no archived primary document behind any headline figure; most registered sources are journalism; 14 CONFIRMED rows are AI-compilation-sourced; geo-verification artifact states "No coordinates detected… 0 verified."
**Required owner decisions:** publish or de-link the master file (D-3); correct the 91-source count and its page footers (D-9); log the qualifying corrections (D-6); resolve the comparator/IPV/tribunal contradictions (editorial); decide the Gemini-doc CONFIRMED rows' handling (editorial); either build the described tracker pipeline or fix its presentation (D-9-adjacent).
**Recommended improvements (presentation-only):** per-figure source tags; data-table fallbacks for canvas charts; public evidence digest. **Implemented this phase where safe:** evidence-status panel + NewsArticle JSON-LD + honest evidence-page labels.
...
P0 — REP-031 impunity master evidence file dead-linked 6+ times publicly while the self-audit ticked "all dataset download links point to actual files"
P0 — REP-032 impunity source count 91 published everywhere vs 65 in the actual registry (verified three ways); pages cite unregistered S-26/27/28
P1 — REP-033 impunity zero archived primary documents; evidence ledger's checksummed paths point to a nonexistent assets/BD-INV-002 folder
P1 — REP-034 impunity page-to-page statistical contradictions (comparators, IPV divisions 5-of-6, tribunal counts, BMP re-periodization, 40-vs-18 acquittals, 8/10/12 measure counts)
P1 — REP-035 impunity pre-publish gate stamped PASS over its own logged Stage 05: 0/3 and Stage 06: 0/12
P2 — REP-036 impunity 14 "Gemini-doc" AI-compilation rows carry CONFIRMED tier (incl. a 165,000 figure contradicting the published pair)
P2 — REP-037 impunity verify_claims.py prints "All claims verified" while testing a fraction and "verifying" a figure published nowhere
P2 — REP-038 impunity tracker: "Updated Weekly" vs own "no fixed schedule"; zero fetch calls; described 6-hourly collector does not exist
P3 — REP-039 impunity period/label drift (2001–2023 vs 2001–2024); version fragmentation (3.0/v5.0/v2.1/v4.1); self-audit formula error
...
- **All four investigation agent audits were delivered in full and incorporated.** Every headline finding from each agent was independently re-verified by the lead auditor before incorporation (blood-routes dataset recomputation and git-log confirmation; lead-belt full 294×9,846 Haversine replication yielding 166/51/125; impunity 65-source registry count, master-file checksum, and evidence-registry path existence; slum-fires empty-hash and hardcoded-script code reads).
