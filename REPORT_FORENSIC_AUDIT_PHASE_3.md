...
|---|---|---|---|---|---|---|---|
| BR-1 | Blood Routes | "351 deaths, 15-day Eid 2026, 23.4/day" | RSF/BJKS Eid-2026 reports (per pipeline `03-data-analysis.md` C001) | **None** — public dataset has zero 2026 records | D | No | Headline sourced to two advocacy-NGO reports; not derivable from any published artifact |
| BR-2 | Blood Routes | WHO ~31,578 vs BRTA 5,480 (5.8×) | WHO Global Status Report 2023; BRTA annual report (pipeline C-xx with sources named) | None public | B | No (external reports not archived) | Well-sourced, standard comparison; neither report archived; hero honestly labels it "modelled" |
| BR-3 | Blood Routes | "47% of Dhaka's road victims are pedestrians" | RSF (per pipeline) | Public dataset: 5.8% of Dhaka deaths pedestrian-involved (87/1,512) | D | Contradicted by published data | Dataset is 85% truck–motorcycle records; claim cannot come from it and isn't caveated |
| BR-4 | Blood Routes | joma incentive system; Phoenix rebrand; 4 data voids | Pipeline narrative + report prose | None | C/D | No | Structural claims reported as analysis; sources named in prose but unarchived |
...
| LB-2 | Lead Belt | 145 schools within 500 m | Embedded per-site school counts (sum = 145, verified) | Embedded + osm_schools.geojson | A **with definitional caveat** | Yes | 145 = **site-school intersections, not unique schools** (121 unique for 44 headline sites; 132 unique across 294); methodology page publishes this reconciliation honestly |
| LB-3 | Lead Belt | "closest school 59 metres" (Kamrangir Char, BD-4591) | Embedded `cm` field = 59 for BD-4591 | Embedded | B | Yes | Verified for BD-4591; **but embedded data contains BD-7303 with `cm`=14 m** — a closer school exists in the bureau's own data; audit's own language ("closest school in the report") is contradicted |
...
| LB-6 | Lead Belt | 26 of 44 sites "active" March 2026 | Gemini 2.0 Flash Lite satellite-tile classification; satellite_proof JPGs archived | `sat` flags in embedded data | C | No | Report qualifies as "visual indicators" and names the tool — good practice; the bureau's own decision memo rated the underlying activity-status claim **Unverified** pre-publication |
...
**Strong:** genuine raw-collection archive (6 source datasets, download script, OS repo); scripted cleaning with lineage; honest hero labeling ("modelled estimate" for WHO figure); geo-verification report exists; macro comparison (5.8×) is well-sourced to named external authorities.
**Weak:** BR-1/BR-3 (headline and 47% claims not derivable from published data; 47% actually contradicted by it); no published distinction between dataset-derived and externally-cited numbers; no dataset caveats in-page (the dataset is 85% truck-motorcycle records — a sampling-profile fact a reader cannot know).
**Critical findings:** REP-013 (BR-3, P1: published data contradicts a cited claim without caveat); REP-014 (BR-1, P2: headline rests on two NGO reports, unarchived).
**Required owner decisions:** whether to publish Eid-2026 incident records (if any exist) or annotate the hero with its external-source basis; whether to publish a dataset-profile caveat.
**Recommended improvements (presentation-only):** source-basis labeling on hero numbers; dataset profile disclosure in methodology; archive the external reports (links) in the evidence system.
...
**Strong:** self-audit suite (`research/lead-belt-audit/`) of unusual honesty — school-count reconciliation, student-estimate audit with bounds, decision memo that *delayed publication*; methodology page publishes the 145-intersections vs 121-unique reconciliation and the estimate's projection nature; replication script shipped at `/methods/analyze.py`; satellite claims qualified with tool named; embedded dataset verified (294 sites, 145 sum, 680,872 ppm/1,702×, BD-4591 cm=59).
**Weak:** hero/meta "39,875 children at extreme risk" outruns the estimate's own published bounds (24,650–50,750; unique-count 33,275 per its own audit); BD-7303's 14 m school closer than the "closest 59 m" narrative; v5 master dataset referenced publicly exists nowhere; manifest date 2025-01-01 conflicts with pipeline activity 2026-05/06.
**Critical findings:** REP-015 (P2: hero framing vs own audit bounds); REP-016 (P3: 14 m record vs "closest 59 m" claim); REP-017 (P1: v5 dataset missing from all repos while publicly referenced).
**Required owner decisions:** publish the v5 dataset (it's described as publication-ready in the decision memo) or de-link; whether the hero adopts the bounds/unique-count framing (editorial); manifest date correction (editorial metadata).
**Recommended improvements:** hero-adjacent "how this estimate works" disclosure fed by the methodology's own text; evidence panel linking analyze.py + osm_schools.geojson.
...
P1 — REP-005 evidence-download system advertises verification it cannot deliver (2 private-referenced, 2 nonexistent)
P1 — REP-006 corrections doctrine breached: qualifying post-publication amendments unlogged (impunity disclosures; slum-fires number changes)
P1 — REP-007 slum-fires dead verification-drawer promise (incl. Phase-2 regression)
P1 — REP-008 Blood Routes 47% pedestrian claim contradicted by published dataset (5.8%), uncaveated
P1 — REP-009 PP methodology note false on its face (268/May–Dec-2025 vs 480/Nov25–Jun26 auto-generated)
P1 — REP-010 AI-use policy vs observable publication practice (impunity file private; see §14)
P2 — REP-011 nominal source counts; homepage "127 sources" without definition
P2 — REP-012 Lead Belt hero framing vs own audit bounds; 14 m vs 59 m closest-school record
...
P3 — REP-016 tier-label taxonomy mismatch; manifest date conflicts; slum-fires governance templates empty
P4 — housekeeping (PP relative links; local file:// references in specs; etc.)
...
- Impunity Machine and Blood Routes agent audits were still consolidating at publication of this document; their headline findings incorporated here were independently re-verified by the lead auditor (blood-routes dataset recomputation; impunity chain inspection). Any late-arriving details will be appended in a revision note rather than silently merged.
